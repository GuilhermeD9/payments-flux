...
import (
	...
	"github.com/GuilhermeD9/payments-flux-go/internal/transfer"
...
func main() {
	...
	service := transfer.NewService(repo, redisClient, cfg.WalletServiceURL)
	...
	addr := "127.0.0.1:" + cfg.TransferServicePort
	srv := &http.Server{
		Addr:         addr,
		Handler:      r,
	...
	go func() {
		...
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.Fatalf("Transfer Service failed: %v", err)
		}
	}()
	...
	log.Println("Transfer Service shutting down...")
	ctx, cancel = context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	if err := srv.Shutdown(ctx); err != nil {
		log.Fatalf("Transfer Service forced to shutdown: %v", err)
	}
	log.Println("Transfer Service stopped")
}
